...
class Tracking:
    ...
    def __init__(self, subject: str): 
        ...
        self.useful_df.loc[:, 'unit'] = self.useful_df['date'].astype(str) + "_" + self.useful_df['unit_code']
    ...
    def calc_similarity_threshold(self, predetermine: bool = True):
        ...
        if predetermine:
            match self.subject:
                ...
                case 'Mouse1':
                    raise ValueError('Thresholds for Mouse1 are not computed yet.')
                case 'Mouse2':
                    raise ValueError('Thresholds for Mouse1 are not computed yet.')
                case 'Mouse3':
                    raise ValueError('Thresholds for Mouse1 are not computed yet.')
            ...
            while len(dist) < 5000:
                ...
                for l in loc:
                    if subj_ch_session[int(l[0]), int(l[1])] != 0:
                        used_pair.append((int(self.channels[int(l[0])]), 
                                        self.dates[int(l[1])]))
